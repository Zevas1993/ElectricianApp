...
echo ===================================
echo ElectricianApp Ultimate Fix
echo ===================================
echo.
...
echo Step 2: Fixing the settings.gradle file
echo ---------------------------------

echo // Top-level settings file > settings.gradle.temp
echo rootProject.name = 'ElectricianApp'>> settings.gradle.temp
echo.>> settings.gradle.temp
echo include ':app'>> settings.gradle.temp
echo include ':data'>> settings.gradle.temp
echo include ':domain'>> settings.gradle.temp
echo include ':feature:box'>> settings.gradle.temp
echo include ':feature:conduit'>> settings.gradle.temp
echo include ':feature:dwelling'>> settings.gradle.temp
echo.>> settings.gradle.temp
...
move /y settings.gradle.temp settings.gradle > nul
echo.

echo Step 3: Fixing the build.gradle file
echo ---------------------------------
...
echo }>> build.gradle.temp
echo.>> build.gradle.temp
echo // Fix for sourceset issues - only apply to Android projects>> build.gradle.temp
...
echo Step 4: Updating gradle.properties with correct JDK path
echo ----------------------------------------------------
...
call gradlew.bat clean assembleDebug --no-daemon --info -Dorg.gradle.java.home="%JDK_PATH%"
